...
//                    if(et_ans1.getText().toString().getBytes().length<=0 || et_ans2.getText().toString().getBytes().length<=0 || et_ans3.getText().toString().getBytes().length<=0 )
                    ...
                    { // Edit_Text에 입력이 되지 않았을 때 // || (or)을 사용해 하나라도 성립( null 값을 받으면 )하면 Toast를 띄운다
                        ...
                        if( Integer.parseInt(et_ans1.getText().toString()) == 9 ) et_ans1.setBackgroundResource(R.drawable.cube_green);
                        else et_ans1.setBackgroundResource(R.drawable.cube_red); // 답이 틀리면
                        if( Integer.parseInt(et_ans2.getText().toString()) == 2 ) et_ans2.setBackgroundResource(R.drawable.cube_green);
                        else et_ans2.setBackgroundResource(R.drawable.cube_red); // 답이 틀리면
                        if( Integer.parseInt(et_ans3.getText().toString()) == 8 ) et_ans3.setBackgroundResource(R.drawable.cube_green);
                        else et_ans3.setBackgroundResource(R.drawable.cube_red); // 답이 틀리면

                        btn_next.setText("문제 풀러 가기");
                        tv_instruction.setText(R.string.cal_example2);
                    }
                }
